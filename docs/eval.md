...
`mcp-doctor` never calls your tools at all. Just selection, which is the step that fails
...
```bash
mcp-doctor eval ./your-server --init    # draft a suite; then edit it and commit it
mcp-doctor eval ./your-server           # run it
mcp-doctor eval ./your-server -v        # every failing case, with the prompt
mcp-doctor eval ./your-server --offline # replay the cache; no model, no cost
```
