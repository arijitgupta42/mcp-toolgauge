...
`mcp-doctor` is a terminal tool, and two of its best outputs do not fit a terminal. The
...
The dashboard reads a `mcp-doctor ci --json` document. It gets one of three ways:
...
```bash
mcp-doctor ci ./path/to/your/server --json > report.json
```

and, for the history view, keep appending to a history file across runs:

```bash
mcp-doctor ci ./path/to/your/server --history history.json --json > report.json
```
...
Pages serves from a repo subpath, that build sets `VITE_BASE=/mcp-doctor/`. Serving from a
domain root instead is `VITE_BASE=/`.

## A note on the demo data

The two bundled reports in `dashboard/public/reports/` are real `mcp-doctor ci` output, not
