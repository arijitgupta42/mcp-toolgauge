...
[private vulnerability reporting](https://github.com/arijitgupta42/mcp-toolgauge/security/advisories/new)
...
`mcp-toolgauge` connects to MCP servers and reads their tool definitions. The parts worth
scrutiny:

- **The connection layer** — it starts a server you point it at (a subprocess for stdio, an
  HTTP client for a URL). `mcp-toolgauge` is read-only by design: it lists tools and disconnects,
...
- Findings from linting a deliberately malicious server. `mcp-toolgauge` reports on a server's
