...
Discovery walks up from the target, checking `mcpcheckup.toml` and then `pyproject.toml` at
...
from mcpcheckup.lint.engine import rule_ids
from mcpcheckup.model import Severity

CONFIG_FILENAME = "mcpcheckup.toml"
PYPROJECT_FILENAME = "pyproject.toml"
PYPROJECT_SECTION = "mcpcheckup"
...
def find_config(start: Path) -> Path | None:
    ...
    A `pyproject.toml` only counts when it actually declares a `[tool.mcpcheckup]` section,
