...
from mcpcheckup import version as package_version
from mcpcheckup.connect import (
...
from mcpcheckup.history import HistoryError
from mcpcheckup.history import record as record_history
from mcpcheckup.lint import ConfigError, load_config
...
def _version_callback(requested: bool) -> None:
    if requested:
        out().print(f"mcpcheckup {package_version()}")
...
def inspect(
    ...
    This is read-only: mcpcheckup lists the tools and disconnects. It never calls one.
    """
...
def lint(
    ...
    no_config: Annotated[
        bool, typer.Option("--no-config", help="Ignore any mcpcheckup.toml on disk.")
    ] = False,
) -> None:
