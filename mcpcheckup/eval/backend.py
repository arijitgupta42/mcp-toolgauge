...
base package -- `uvx mcpcheckup lint` is the command that runs on every pull request and it
must stay fast to install. And `mcpcheckup eval --offline`, which replays a recorded cache
...
from mcpcheckup.model import ServerInfo, ToolSpec

# A free model, so that `mcpcheckup eval` costs a new user nothing on the first run. Pass
...
def _require_litellm() -> Any:
    try:
        ...
        raise BackendUnavailable(
            ...
            "  Install it with:  uv pip install 'mcpcheckup[eval]'\n"
            "  Or run against a recorded cache with:  mcpcheckup eval <target> --offline"
        ) from exc
