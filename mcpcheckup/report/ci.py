...
from mcpcheckup.eval.score import describe_confusion, notable_confusions
from mcpcheckup.model import CiReport, EvalResult, HealthScore, Severity, at_least, canonical_json
...
_REPO_URL = "https://github.com/arijitgupta42/mcpcheckup"
...
def render_ci_markdown(report: CiReport, *, baseline: CiReport | None = None) -> str:
    ...
    lines: list[str] = [
        f"## {_EMOJI[_tone(health.overall)]} mcpcheckup health: {health.overall} / 100",
        "",
    ...
    if health.is_lint_only:
        ...
        lines.append(
            ...
            "Add one with `mcpcheckup eval . --init`."
        )

    lines.append("")
    lines.append(f"<sub>Measured by [mcpcheckup]({_REPO_URL}).</sub>")
    return "\n".join(lines)
