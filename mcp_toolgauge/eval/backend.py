...
base package -- `uvx mcp-toolgauge lint` is the command that runs on every pull request and it
must stay fast to install. And `mcp-toolgauge eval --offline`, which replays a recorded cache
...
from mcp_toolgauge.model import ServerInfo, ToolSpec

# A free model, so that `mcp-toolgauge eval` costs a new user nothing on the first run. Pass
...
def _require_litellm() -> Any:
    try:
        ...
        raise BackendUnavailable(
            ...
            "  Install it with:  uv pip install 'mcp-toolgauge[eval]'\n"
            "  Or run against a recorded cache with:  mcp-toolgauge eval <target> --offline"
        ) from exc
