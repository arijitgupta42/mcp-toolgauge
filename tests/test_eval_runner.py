...
from mcp_doctor.eval.backend import BackendError, Completion, ToolCall, build_messages
from mcp_doctor.eval.cache import CachedCall, CacheMiss, ResponseCache, cache_key
from mcp_doctor.eval.runner import (
    BACKOFF_SECONDS,
    MAX_ATTEMPTS,
    MAX_BACKOFF_SECONDS,
    BudgetExceeded,
...
from mcp_doctor.model import CaseKind, CaseSuite, EvalCase, ServerInfo, ToolSpec, tool_digest
...
class TestTextCompleter:
    ...
    def test_a_second_call_is_served_from_the_cache(
        ...
        monkeypatch.setattr("mcp_doctor.eval.backend.complete_text", fake)
    ...
    def test_a_rate_limit_is_retried(
        ...
        monkeypatch.setattr("mcp_doctor.eval.backend.complete_text", flaky)
