...
`mcp-doctor.toml` in some ancestor of the checkout cannot quietly change what they prove.
"""
...
from mcp_doctor.cli import EXIT_CONNECTION, EXIT_OK, EXIT_THRESHOLD, EXIT_USAGE, app
...
class TestConfiguration:
    def test_a_config_can_silence_a_rule(self, badserver_dir: Path, tmp_path: Path) -> None:
        config = tmp_path / "mcp-doctor.toml"
    ...
    def test_verbose_names_the_config_in_use(self, goodserver_dir: Path, tmp_path: Path) -> None:
        ...
        assert "mcp-doctor.toml" in result.output
