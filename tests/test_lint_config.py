...
from mcp_doctor.lint import ConfigError, find_config, load_config
from mcp_doctor.model import Severity
...
class TestDiscovery:
    def test_a_config_beside_the_target_is_found(self, tmp_path: Path) -> None:
        write(tmp_path, "mcp-doctor.toml", OWN)

        assert find_config(tmp_path) == tmp_path / "mcp-doctor.toml"
...
class TestLoading:
    ...
    def test_fail_on_defaults_to_error(self, tmp_path: Path) -> None:
        write(tmp_path, "mcp-doctor.toml", "[lint.rules]\nMCP025 = 'off'\n")
    ...
    def test_an_explicit_path_skips_discovery(self, tmp_path: Path) -> None:
        ...
        write(tmp_path, "mcp-doctor.toml", "[lint]\nfail_on = 'info'\n")
    ...
    def test_the_source_is_reported(self, tmp_path: Path) -> None:
        path = write(tmp_path, "mcp-doctor.toml", OWN)
...
class TestErrors:
    ...
    def test_malformed_toml_names_the_file(self, tmp_path: Path) -> None:
        path = write(tmp_path, "mcp-doctor.toml", "[lint\nbroken")
    ...
    def test_a_wrong_shaped_lint_section_is_an_error(self, tmp_path: Path) -> None:
        write(tmp_path, "mcp-doctor.toml", "lint = 'yes please'\n")
