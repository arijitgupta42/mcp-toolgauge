...
Discovery walks up from the target, checking `mcp-doctor.toml` and then `pyproject.toml` at
...
from mcp_doctor.lint.engine import rule_ids
from mcp_doctor.model import Severity

CONFIG_FILENAME = "mcp-doctor.toml"
PYPROJECT_FILENAME = "pyproject.toml"
PYPROJECT_SECTION = "mcp-doctor"
...
def find_config(start: Path) -> Path | None:
    ...
    A `pyproject.toml` only counts when it actually declares a `[tool.mcp-doctor]` section,
