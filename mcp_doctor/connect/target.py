...
The resolution order is deliberate. Most people pointing mcp-doctor at a project already
have an `.mcp.json` there, because that is what their MCP client reads. Honouring it means
`mcp-doctor inspect .` works with no flags in a directory that already works elsewhere --
...
def _servers_in(manifest: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    raise TargetResolutionError(
        ...
        "Add one, or point mcp-doctor at the server directly with --command."
    )
...
def _pick_server(servers: dict[str, Any], path: Path, requested: str | None) -> tuple[str, Any]:
    ...
    if len(servers) > 1:
        available = ", ".join(sorted(servers))
        raise TargetResolutionError(
            f"{path} declares {len(servers)} servers, so mcp-doctor cannot guess which one "
...
def _resolve_directory(directory: Path, server: str | None) -> ServerTarget:
    ...
    for entrypoint in CONVENTION_ENTRYPOINTS:
        ...
        if candidate.is_file():
            ...
            # depend on the interpreter that mcp-doctor itself is running under.
...
def resolve_target(
    ...
    *,
    ...
    if path.is_file():
        ...
        raise TargetResolutionError(
            f"{path} is not a file mcp-doctor knows how to start. Expected a .py server or a "
