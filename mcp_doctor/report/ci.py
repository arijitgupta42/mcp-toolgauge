...
from mcp_doctor.model import CiReport, EvalResult, HealthScore, Severity, at_least, canonical_json
...
_REPO_URL = "https://github.com/arijitgupta42/mcp-doctor"
...
def render_ci_markdown(report: CiReport, *, baseline: CiReport | None = None) -> str:
    ...
    lines: list[str] = [
        f"## {_EMOJI[_tone(health.overall)]} mcp-doctor health: {health.overall} / 100",
        "",
    ...
    if health.is_lint_only:
        ...
        lines.append(
            ...
            "Add one with `mcp-doctor eval . --init`."
        )

    lines.append("")
    lines.append(f"<sub>Measured by [mcp-doctor]({_REPO_URL}).</sub>")
    return "\n".join(lines)
